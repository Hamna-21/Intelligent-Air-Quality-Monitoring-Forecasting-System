...
import { verifyPollutionIncident, generateComplaintLetter } from '../../services/api';
...
export const PollutionReporterView: React.FC = () => {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files.length > 0) {
            ...
            files.forEach(file => {
                 // TEST CASE: Unsupported file format
                if (!file.type.startsWith('image/')) {
                    setError("Unsupported file format skipped. Please use JPG/PNG.");
                    return;
                }
                 // TEST CASE: Large file upload (>10MB)
    ...
    const handleAnalyzeAll = async () => {
        ...
        for (const item of pendingItems) {
            try {
                const analysis = await verifyPollutionIncident(item.image);
    ...
    return (
        <div className="h-full flex flex-col animate-fade-in pb-10">
            <h1 className="text-3xl font-bold font-heading text-slate-900 mb-2">Eco-Enforce <span className="text-emerald-500 text-lg align-middle bg-emerald-50 px-2 py-1 rounded-lg border border-emerald-100 ml-2">Beta</span></h1>
            <p className="text-slate-500 mb-8 max-w-2xl">AI-powered legal action center. Upload evidence (smoke, burning, industrial waste) to generate verified legal complaints.</p>

            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                {/* Left: Upload Queue */}
                <div className="lg:col-span-4 space-y-6">
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                        <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">Incident Location</label>
                        <input 
                            type="text" 
                            value={locationInput}
                    ...
                    <div className="bg-slate-50 border-2 border-dashed border-slate-300 rounded-2xl p-6 text-center hover:bg-slate-100 transition-colors cursor-pointer relative">
                        ...
                        <div className="flex flex-col items-center">
                            <svg className="w-10 h-10 text-slate-400 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 4v16m8-8H4" /></svg>
                            <span className="text-slate-700 font-bold text-sm">Add Evidence Photos</span>
                            <span className="text-slate-400 text-xs mt-1">Supports Multiple Files</span>
                        </div>
                    </div>
                    ...
                    {uploads.length > 0 && (
                        ...
                        >
                            {uploads.some(u => u.status === 'analyzing') ? 'Processing Evidence...' : `Analyze ${uploads.length} Images`}
                        </button>
                    )}
                </div>
                ...
                <div className="lg:col-span-8 space-y-6">
                    {uploads.filter(u => u.status === 'done' && u.result).length === 0 && (
                    ...
                    {uploads.filter(u => u.status === 'done' && u.result).map((item) => (
                        <div key={item.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col md:flex-row animate-fade-in">
                            <div className="md:w-1/3 relative h-48 md:h-auto">
                                ...
                                <div className={`absolute top-3 left-3 px-3 py-1 rounded-full text-xs font-bold border shadow-sm ${item.result?.isViolation ? 'bg-red-50 text-red-700 border-red-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'}`}>
                                    {item.result?.isViolation ? 'Violation Confirmed' : 'No Violation'}
                                </div>
                            </div>
                            <div className="p-6 md:w-2/3 flex flex-col justify-between">
                                <div>
                                    <div className="flex justify-between items-start mb-2">
                                        <h3 className="font-bold text-lg text-slate-900">{item.result?.violationType}</h3>
                                        {item.result?.isViolation && <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Confidence: {item.result?.confidence}%</span>}
                                    </div>
                                    <p className="text-slate-600 text-sm mb-4 leading-relaxed">{item.result?.description}</p>
                                    ...
                                    {item.result?.isViolation && (
                                        <div className="flex gap-4 mb-4">
                                             <div className="bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100">
                                                ...
                                                <span className={`text-sm font-bold ${item.result.severity === 'Critical' ? 'text-red-600' : 'text-slate-800'}`}>{item.result.severity}</span>
                                             </div>
                                        </div>
                                    )}
                                </div>
